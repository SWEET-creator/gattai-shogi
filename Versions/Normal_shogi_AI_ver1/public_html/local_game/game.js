...
function CP(){
	...
	let choosed_koma;
	let pos = undefined;
	let simu_koma_list = copy_obj(koma_list);
	let koma_list_save = koma_list;
	koma_list = simu_koma_list;
	//駒の決定
	search()
	koma_list = koma_list_save;
...
function copy_obj(list){
	let copied_list = [];
	koma_list.forEach(e => {
		let koma = Object.create(e);
		koma.x = e.x;
		koma.y = e.y;
		koma.side = e.side;
		koma.stock_flag = e.stock_flag;
		copied_list.push(koma);
	});
	return copied_list;
}
...
function search(){
	...
	koma_list.forEach(koma => {
		...
		mova.forEach(pos => {
			let simu_koma_list = copy_obj(koma_list);
			let koma_list_save = koma_list;
			let koma_list = simu_koma_list;
			koma.movable(pos[0], pos[1]);
			koma_list = koma_list_save;
			tree[[koma, pos]] = evaluate();
		});
	});
	tree.map(function(o){return o.})
}

function evaluate(){
	let y = 0
	koma_list.forEach(koma => {
		if (koma.stock_flag == true && koma.side == 0){
			y ++;
		}else if(koma.stock_flag == true && koma.side == 1){
			y --;
		}
	});
	return y;
