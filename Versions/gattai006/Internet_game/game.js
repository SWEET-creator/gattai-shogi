			...
			if (target_obj.side != this.side){
				...
				Object.keys(gattai_koma_list).forEach( function(key) {
					...
					if (value[0] == selected_obj.constructor.name && value[1] == target_obj.constructor.name
						...
						let maked_koma = make_gattai_koma_functions[key];
						if (selected_obj.side == target_obj.side && maked_koma != undefined){							
							maked_koma.x = selected_obj.x;
							maked_koma.y = selected_obj.y;
							maked_koma.side = selected_obj.side;				
							koma_list.push(maked_koma);							
...
function init(){
	...
	//koma_list.push(new KYKM(3,4,0));
	//koma_list.push(new KNKY(4,4,0));
	//koma_list.push(new GNKM(5,4,0));
...
function AI(){
	...
	let out = random_choice(cp_koma_list);
	selected_obj = out[0];
	let pos = out[1]
...
function random_choice(cp_koma_list){
